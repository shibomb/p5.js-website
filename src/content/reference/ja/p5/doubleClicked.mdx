...
description: >
  <p><a href="#/p5/doubleClicked">doubleClicked()</a> 関数は、DOM L3 仕様の一部である
  dblclick イベントを検出したイベントリスナーがあるたびに実行されます。</p>

  <p>doubleClicked
  イベントは、ポインティングデバイスのボタン（通常はマウスのプライマリボタン）がひとつの要素上で2回クリックされると発火します。dblclick
  イベントの詳細については、mozilla のドキュメントを参照してください: <a
