...
description: >-
  <p>与えられた色でアンビエントライトを作成します。</p>

  <p></p>

  <p>アンビエントライトは特定の方向から来るものではありません。オブジェクトはすべての面から均等に照らされます。アンビエントライトは、ほとんどの場合、他の種類のライトと組み合わせて使用されます。</p>

  <p>注意: ループするプログラムでライトが持続するためには、draw() 内で（直接的または間接的に）呼び出す必要があります。setup()
  に配置すると、ループの最初のサイクルでのみ効果があります。</p>
