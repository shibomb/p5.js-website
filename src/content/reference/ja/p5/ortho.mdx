...
description: >
  <p>3D スケッチの現在のカメラに対して正射影を設定し、オブジェクトが見られる箱型の視錐台を定義します。</p>

  <p>この投影では、同じ次元のすべてのオブジェクトは、カメラに近いか遠いかに関係なく、同じサイズで表示されます。</p>

  <p>この関数のパラメーターは、左と右が最小および最大の x 値、上と下が最小および最大の y 値、および近くと遠くが最小および最大の z
  値である視錐台を指定します。</p>

  <p>パラメーターが指定されていない場合、デフォルトで次のようになります: ortho(-width/2, width/2, -height/2,
  height/2)。</p>
