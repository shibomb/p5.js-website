...
description: >-
  <p>提供された頂点シェーダーとフラグメントシェーダーファイルから新しい <a href="#/p5.Shader">p5.Shader</a>
  オブジェクトを作成します。</p>

  <p></p>

  <p>シェーダーファイルはバックグラウンドで非同期にロードされるため、このメソッドは <a
  href="#/p5/preload">preload()</a> で使用する必要があります。</p>

  <p>Shaders can alter the positioning of shapes drawn with them. To ensure
  consistency in rendering, it's recommended to use the vertex shader in the <a
  href="#/p5/createShader">createShader example</a>. /// todo /// add 1 line</p>

  <p>注意:シェーダーは WebGL モードでのみ使用できます。</p>
  ...
  todo: param failureCallback not found in ja-legacy:
...
params:
  - name: vertFilename
    description: |
      <p>頂点シェーダーソースコードが含まれるファイルへのパス</p>
      <p>path of the vertex shader to be loaded.</p>
    type: String
  - name: fragFilename
    description: |
      <p>フラグメントシェーダーソースコードが含まれるファイルへのパス</p>
...
return:
  description: |-
    提供された頂点シェーダーとフラグメントシェーダーファイルから作成されたシェーダーオブジェクト。
