...
description: >
  <p>3D スケッチ内の現在のカメラの位置を設定します。</p>

  <p>この関数のパラメーターは、カメラの位置、スケッチの中心（カメラが指している場所）、および上方向（カメラの向き）を定義します。</p>

  <p>この関数はカメラの動きをシミュレートし、オブジェクトをさまざまな角度から見ることができます。覚えておいてください、オブジェクト自体を移動させるのではなく、カメラを移動させます。たとえば、centerX
  の値が正であり、カメラがスケッチの右側に回転する場合、オブジェクトは左に移動しているように見えます。</p>

  <p>この <a href = "https://www.openprocessing.org/sketch/740258">例</a>
  を参照して、カメラの位置を確認してください。</p>

  <p>パラメーターが指定されていない場合、デフォルトで次のようになります: camera(0, 0, (height/2) / tan(PI/6), 0,
  0, 0, 0, 1, 0)</p>
