...
description: >
  ...
  shader, as a convenience method for creating image effects. It's like <a
  href="#/createShader">createShader()</a> but with a default vertex shader
  included.。</p>
  ...
  a canvas. A filter shader will not be applied to any geometries.</p>

  <p>The fragment shader receives some uniforms: <ul> <li><code>sampler2D
  tex0</code>, which contains the canvas contents as a texture</li>
  <li><code>vec2 canvasSize</code>, which is the p5 width and height of the
  canvas (not including pixel density)</li> <li><code>vec2 texelSize</code>,
  which is the size of a physical pixel including pixel density
  (<code>1.0/(width*density)</code>, <code>1.0/(height*density)</code>)</li>
  </ul></p>
...
params:
  - name: fragSrc
    description: |
      <p>source code for the fragment shader /// todo /// </p>
      <p>source code for the fragment shader.</p>
    type: String
return:
  description: |-
    a shader object created from the provided  fragment shader. /// todo /// 
