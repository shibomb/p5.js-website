...
file: src/dom/dom.js
description: >-
  <p>引数が与えられていない場合、要素の現在の時間を返します。</p>

  <p>引数が与えられた場合、要素の現在の時間がそれに設定されます。</p>

  <p>/// todo ///</p>

  <p>Manages the media element's playback time. Calling
  <code>media.time()</code> returns the number of seconds the audio/video has
  played. Time resets to 0 when the looping media restarts.</p>

  <p>The parameter, <code>time</code>, is optional. It's a number that specifies
  the time, in seconds, to jump to when playback begins.</p>
