---
title: fromAngles
module: Math
submodule: Vector
file: src/math/p5.Vector.js
description: |
  <p>2つの ISO 球面角から新しい 3D ベクトルを作成します。</p>
  <p></p>

  <hr/>
  <p>Creates a new 3D vector from a pair of ISO spherical angles.</p>
line: 3174
isConstructor: false
itemtype: method
example:
  - |-

    <div class="norender">
    <code>
    function setup() {
      // Create a p5.Vector object.
      let v = p5.Vector.fromAngles(0, 0);

      // Prints "p5.Vector Object : [0, -1, 0]" to the console.
      print(v.toString());
    }
    </code>
    </div>

    <div>
    <code>
    function setup() {
      createCanvas(100, 100, WEBGL);

      describe('A light shines on a pink sphere as it orbits.');
    }

    function draw() {
      background(0);

      // Calculate the ISO angles.
      let theta = frameCount *  0.05;
      let phi = 0;

      // Create a p5.Vector object.
      let v = p5.Vector.fromAngles(theta, phi, 100);

      // Create a point light using the p5.Vector.
      let c = color('deeppink');
      pointLight(c, v);

      // Style the sphere.
      fill(255);
      noStroke();

      // Draw the sphere.
      sphere(35);
    }
    </code>
    </div>
class: p5.Vector
params:
  - name: theta
    description: >
      <p>polar angle in radians (zero is up). /// todo /// Number:
      極角（単位は弧度法、上を0度とします）</p>

      <p>polar angle in radians (zero is up).</p>
    type: Number
  - name: phi
    description: >
      <p>azimuthal angle in radians  (zero is out of the screen). /// todo ///
      Number: 方位角（単位は弧度法、スクリーンから外側を0度とします）</p>

      <p>azimuthal angle in radians
                                    (zero is out of the screen).</p>
    type: Number
  - name: length
    description: |
      <p>(オプション) 新しいベクトルの長さ（デフォルトは1）</p>
      <p>length of the new vector (defaults to 1).</p>
    type: Number
    optional: true
return:
  description: |-
    新しい <a href="#/p5.Vector">p5.Vector</a> オブジェクト
    new <a href="#/p5.Vector">p5.Vector</a> object.
  type: p5.Vector
chainable: false
---


# fromAngles
