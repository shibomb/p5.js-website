...
description: >-
  <p>ベクトルから <code>x</code>、<code>y</code>、<code>z</code> 成分を引きます。</p>

  <p>また、ひとつのベクトルから別のベクトルを引き、2つの独立したベクトルを引くこともできます。2つのベクトルを引くメソッドは静的メソッドで、<a
  href="#/p5.Vector">p5.Vector</a>
  を返します。その他のメソッドはベクトルに直接作用します。このメソッドに引数を配列として提供することもできます。詳細については、例を参照してください。</p>

  <p>/// todo ///</p>

  <p>Subtracts from a vector's <code>x</code>, <code>y</code>, and
  <code>z</code> components using separate numbers, another <a
  href="#/p5.Vector">p5.Vector</a> object, or an array of numbers. Calling
  <code>sub()</code> with no arguments has no effect.</p>

  <p>The static version of <code>sub()</code>, as in <code>p5.Vector.sub(v2,
  v1)</code>, returns a new <a href="#/p5.Vector">p5.Vector</a> object and
